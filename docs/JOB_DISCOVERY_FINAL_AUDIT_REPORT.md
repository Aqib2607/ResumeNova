...
1. **`candidate_skills` Table vs Model:**
   ...
   - *Impact:* The candidate skills feature uses `name` in code while migration defined `skill_name`.
...
2. **`job_preferences` Table vs Model:**
   ...
   - *Impact:* Extended preference fields in the controller are not yet reflected in the initial migration columns.
...
3. **`job_applications` Table vs Model:**
   ...
   - *Impact:* Controller validates `resume_id`, but column does not exist in the initial migration.
...
*Note: None of these discrepancies impact the Core Job Discovery, Search & Filter, or AI Matching pipelines.*
...
*Report Generated and Saved to `D:\ResumeNova\docs\JOB_DISCOVERY_FINAL_AUDIT_REPORT.md`*
